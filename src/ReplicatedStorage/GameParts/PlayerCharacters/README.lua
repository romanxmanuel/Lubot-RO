--[[
PLACE:
ReplicatedStorage.GameParts.PlayerCharacters

WHAT THIS IS:
Character skin templates used by CharacterSkinService.

CURRENT TEMPLATES:
- DekuCharacterTemplate (default)
- GojoWhiteCharacterTemplate (import source asset id 91131914057854)
- LuffyCharacterTemplate (import source asset id 9465786796)
- LuffyGear5CharacterTemplate (import source asset id 10039008438)

NOTES:
- Keep each imported skin source package organized under ReplicatedStorage.GameParts.ImportedAssets.Skins.
- CharacterSkinService can also lazily load a template by skinAssetId if the template model is missing.
]]

return {}
